...
Tsurugiでは、トランザクション開始時にトランザクションモードを指定する必要がある。
...
オンライン処理はOCCでトランザクションを実行する。
トランザクションがabortした場合はそのトランザクションを再実行するが、その際のトランザクションモードは、abortの原因となった競合トランザクションがオンライン処理（単時間で終わる処理）であれば、再実行の際もOCCで良い。
しかしそういった判定は出来ないという前提で、バッチモードで再実行する。（処理内容が照会系（更新が無い）であればread only、それ以外はread-write）

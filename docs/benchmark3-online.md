...
- リトライ時もOCC
- リトライ時もLTX（selectのみのトランザクションはRTX）
- 何回かOCCでリトライし、LTX（RTX）に切り替える
...
1. 在庫テーブルをクリアする。
   `delete from 在庫テーブル where 日付 = 当日日付`
2. 原価マスターから取得する。（品目毎に複数工場分のレコードが存在する）
   `select * from 原価マスター order by 品目ID`
3. 品目毎に、在庫単位を統一して在庫数量・在庫金額を集計する。
   - 在庫単位の統一先は、重量はkg、容量はL、長さはm
4. 在庫テーブルに登録する。
   `insert into 在庫テーブル values(当日日付, 品目ID, …)`
5. コミットする。

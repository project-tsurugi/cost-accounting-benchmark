...
品目マスターは、INSERT時に追加でさらに2レコード（ランダムで日付的に前か後ろに）INSERTする。
...
パラメーターで指定された中間材品目数の1/10個の木を作成し、そこから中間材の品目マスターおよび品目構成マスターを生成する。

ひとつの木は以下のように作成する。

1. 木の要素数を決定する。10+ランダム[-4 .. 4]個（★TODO ランダムだと件数にぶれが出てしまうが、いいか？）
   - 木の要素数が平均n個で、指定された品目数の1/n個の木を作れば、全要素数は指定された品目数になる。
2. ルート要素を作成する。
...
6. 葉要素に重量を割り当てる。
   ...
   2. 重量を子要素に対してランダムな割合で分割して割り当てる。
      再帰的に子要素に対して実施する。
8. 木構造のそれぞれの要素（ルート以外）に対し品目構成マスターを生成する。
...
B)
300×工場数を各工場にランダムで割り当てる（1工場当たり平均300件となる）。
工場毎に、Aと重複しない品目IDを品目マスターの製品の中から割り当てられた数だけランダムに選択する。
...
有効日付範囲を持つテーブルに対し、レコード数の増幅を行う。

テーブル毎に所定の割合でレコードを選択し、それを元として有効範囲日付を変更したレコードをINSERTする。
その際、単純に乱数で値を決めたカラムに関しては、同じくランダムに値を決定する。それ以外のカラムは元となるレコードと同じ値とする。

- 品目マスター3倍（品目マスター生成時に増幅済み）（★MEMO AN：50万→150万）

...
オンライン処理は、1スレッド（★TODO スレッドに替わる表現（TPC-Cの「端末」相当））につきひとつの工場のみを担当する。（そのスレッドが扱う工場IDを固定する。別スレッドが同じ工場IDを扱うことは無い）
★TODO 工場の想定件数は60件なので、1スレッド1工場だとスレッド数が多くなりすぎる？1スレッド複数工場で固定？そもそも60件全てが実行対象か？
...
★TODO 実行結果（特に照会系）はログファイルに出力？（TPC-Cのような「端末」は規定しない）
...
4. 在庫を減らす場合
   ...
   2. 在庫数量 - 減らす在庫数 が0以下になる場合、在庫を増やす処理に切り替える（前述の処理）。
   3. 原価マスターを更新する。
      ```
      update 原価マスター

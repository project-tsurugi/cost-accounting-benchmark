 ...
 * <p>
 * <ul>
 * <li>カバー率の指定: テーブル全体の中から指定した割合のデータを選択対象にします。テーブルの全レコードのうち、カバー率の割合のレコードのみをRandomKeySelectorで管理することにより実現します。</li>
 * <li>at least once: 一定回数以上のランダムな選択を行った場合に、選択対象のレコードが少なくとも1回は選択されることを保証します。</li>
...
public class RandomKeySelector<T> {
     ...
     * 指定のKeyを削除する.
     * <p>
     * getAndRemoveを使用せずにdeleteを実行する場合に、deleteしたレコードのキーを RandomKeySelectorの管理対象外にするために使用する
     *
     * @param key
    ...
    public RandomKeySelector(@Nonnull Collection<T> keys, @Nonnull Random random, double aloSelectRate, double coverRate) throws IllegalArgumentException {
        ...
        initKeyPostionMap();
        initAloKeyList();
        LOG.debug("Inialized with {} keys.", keys.size());
    }
